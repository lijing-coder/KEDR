...
#                     jac_loss = jac_loss_estimate(new_z2, z2)
    ...
    def forward(self, features):
        ...
        """
        # Increment the counter each time forward is called
        self.call_count += 1
        ...
        #print(f"DEQFusion was called ",self.call_count)
...
class Base_Model(nn.Module):
    ...
    def __init__(self,num_classes):
        ...
        self.decision_branch = nn.Linear(1024 * 1 * 2, num_classes)
        self.deq_fusion = DEQFusion(2048, 2)
    ...
    # def forward(self, inputs):
        ...
        fusion_feature,_,_ = self.deq_fusion([b1, b2])
        #fusion_feature = b1 + b2
        logit = self.decision_branch(fusion_feature)

        return logit, fusion_feature
        
if __name__ == '__main__':
    num_classes = 5
    model = Base_Model(num_classes).cuda()
    out = model(torch.ones((32,3,224,224)).cuda(),torch.ones((32,3,224,224)).cuda())
    print(out[1].shape)